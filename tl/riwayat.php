...
// 1. Riwayat Transaksi (hanya agen milik TL)
$transaksi = mysqli_query($koneksi, "
    SELECT t.*, u.nama_lengkap AS nama_agen
    FROM transaksi t
    JOIN users u ON t.agen_id = u.id
    WHERE u.tl_id = $tl_id_session
    ORDER BY t.created_at DESC
");

// 2. Riwayat Request Stok (hanya agen milik TL)
...
<head>
    ...
    <title>Riwayat - Panel Team Leader</title>
    ...
    <div class="d-flex" id="main-wrapper">
        ...
        <div class="flex-grow-1 p-4">
            <h3 class="page-title mb-1">
                <i class="bi bi-clock-history me-2 text-primary"></i> Riwayat Tim
            </h3>
            <p class="text-muted small mb-4">Riwayat seluruh transaksi dan permintaan stok dari agen di bawah Anda.</p>
            ...
            <ul class="nav nav-tabs mb-4" id="riwayatTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active fw-semibold" id="transaksi-tab" data-bs-toggle="tab" data-bs-target="#transaksi" type="button" role="tab" style="color:#1a1a2e;">
                        <i class="bi bi-receipt-cutoff me-1"></i> Transaksi Agen
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link fw-semibold" id="stok-tab" data-bs-toggle="tab" data-bs-target="#stok" type="button" role="tab" style="color:#1a1a2e;">
                        <i class="bi bi-boxes me-1"></i> Request Stok Agen
                    </button>
                </li>
            </ul>
                                ...
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <?php if (mysqli_num_rows($transaksi) == 0): ?>
                                            <tr><td colspan="6" class="text-center text-muted py-5">Belum ada riwayat transaksi.</td></tr>
                                        <?php endif; ?>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <?php if (mysqli_num_rows($request_stok) == 0): ?>
                                            <tr><td colspan="5" class="text-center text-muted py-5">Belum ada riwayat request stok.</td></tr>
                                        <?php endif; ?>
                                    </tbody>
                                </table>
                            </div>
